...
func urutAkun(T *arrlogin, n int) {
	var pass, i int
	var temp login
	pass = 1
	for pass <= n-1 {
		i = pass
		temp = T[pass]
		for i > 0 && temp.admin < T[i-1].admin {
			T[i] = T[i-1]
			i--
		}
		T[i] = temp
...
func signinAdmin(T arrlogin, user, pass string) string {
	temp := T
	urutAkun(&temp, 21)
	...
	for kiri <= kanan {
		tengah := (kiri + kanan) / 2
		if temp[tengah].admin == user && temp[tengah].passAdmin == pass {
			return temp[tengah].admin
		} else if temp[tengah].admin > user && temp[tengah].passAdmin > pass {
			kanan = tengah - 1
		} else {
			kiri = tengah + 1
		}
...
func userAdmin(T arrlogin, user, pass string) string {
	fmt.Print("Username : ")
	fmt.Scan(&user)
	fmt.Print("Password : ")
	fmt.Scan(&pass)
	nama := signinAdmin(T, user, pass)
	for nama == "-1" {
		fmt.Println("--------------------------------------------")
...
func TempatWisata(T *arrPariwisata) {
	...
	T[3].kategori = "Hiburan"
	T[3].katakunci = "JatimPark4"
	T[3].name = "JawaTimurPark4"
	T[3].hargatiket = 55000
	T[3].pusat = 25
}
...
func renameWahana(T *arrPariwisata, X string) {
	...
	for found == -1 {
		...
		fmt.Scan(&cari2)
		i = 0
		for i = 0; i < T[W].twahan; i++ {
			if cari2 == T[W].wahan[i].name {
				found = i
			}
		}
	}
	fmt.Println("Rename dengan : <NAMA WAHANA>")
...
func deleteFitur(T *arrPariwisata, X string) {
	...
	var i int = 0
	W := Caristring(*T, wisata, X)
	fmt.Print("Nama FASILITAS yang di HAPUS : ")
	fmt.Scan(&cari2)
	var found int = -1
	for i < T[W].tfasilitas && found == -1 {
		if cari2 == T[W].fitur[i].name {
...
func edit(T *arrPariwisata, d *int) {
	...
	for status2 != "LANJUT" {
		if status2 == "1" || status2 == "Rename Wahana" {
			...
			deleteWahana(T, cari)
		} else if status2 == "3" || status2 == "Rename Fasilitas" {
			renameFiture(T, cari)
		} else if status2 == "4" || status2 == "DELETE Fasilitas" {
			deleteFitur(T, cari)
		}
		fmt.Println()
		fmt.Println("Pilih akan melakukan edit :")
...
func printAdmin(T arrPariwisata) {
	...
	for i := 0; i < wisata; i++ {
		...
		fmt.Printf("%s 			%d fasilitas\n", T[i].name, T[i].tfasilitas)
...
func printChose(T arrPariwisata, d int) {
	...
	fmt.Println("Biaya Tiket Masuk :", T[idx].hargatiket)
...
func Tiket(T *arrPariwisata, n int) {
	...
	for pass <= n-1 {
		...
		if X == "Termahal" {
			for i < n {
				if T[i].hargatiket > T[idx].hargatiket {
					idx = i
				}
				i++
			}
...
func Fasilitas(T *arrPariwisata, n int) {
	var X string
	var pass, i int
	var temp pariwisata
	fmt.Scan(&X)
	...
	for pass <= n-1 {
		...
		temp = T[pass]
		if X == "Terbanyak" {
			for i > 0 && temp.tfasilitas > T[i-1].tfasilitas {
				T[i] = T[i-1]
				i--
			}
		} else if X == "Terendah" {
	...
	printFasilitas(*T, wisata, X)
}
func printFasilitas(T arrPariwisata, n int, X string) {
	fmt.Println("Fasilitas ", X)
	fmt.Println("--------------------------------------------")
	for i := 0; i < wisata; i++ {
		fmt.Println("--------------------------------------------")
		fmt.Printf("%s		 	    %s \n", T[i].katakunci, T[i].kategori)
		...
		if num == "2" || num == "USER" {
			...
			for inapk1 != "LOGOUT" {
				...
				if inapk1 == "1" {
					...
					fmt.Println("2. FASILITAS")
					fmt.Println("3. JUMLAH FASILITAS")
					fmt.Println("4. JARAK")
					fmt.Println("5. HARGA TIKET")
					fmt.Println("LOGOUT")
					fmt.Print("JAWABANMU : ")
					fmt.Scan(&inapk1)
					if inapk1 == "1" || inapk1 == "KATEGORI" {
						...
						fmt.Print("Pilih HOME/LOGOUT : ")
						fmt.Scan(&inapk1)
					} else if inapk1 == "4" || inapk1 == "JARAK" {
						fmt.Println("Melakukan Filter JARAK \nTertinggi/Terendah")
						jarak(&T, wisata)
						printChose(T, d)
						fmt.Print("Pilih HOME/LOGOUT : ")
						fmt.Scan(&inapk1)
					} else if inapk1 == "5" || inapk1 == "HARGA TIKET" {
						fmt.Println("Melakukan Filter HARGA TIKET \nTermahal/Termurah")
						Tiket(&T, wisata)
						printChose(T, d)
						fmt.Print("Pilih HOME/LOGOUT : ")
						fmt.Scan(&inapk1)
					} else if inapk1 == "3" || inapk1 == "JUMLAH FASILITAS" {
						fmt.Println("Melakukan Filter JUMLAH FASILITAS \nTerbanyak/Terendah")
						Fasilitas(&T, wisata)
						printChose(T, d)
						fmt.Print("Pilih HOME/LOGOUT : ")
						fmt.Scan(&inapk1)
